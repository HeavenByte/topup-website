...
<head>
    ...
    <title>Reset Password - QuasarTopUp</title>
...
<body class="bg-slate-950 antialiased relative overflow-hidden">
    ...
    <div class="min-h-screen flex items-center justify-center p-4 sm:p-8 relative z-10">
        ...
        <!-- Main Card Container (Efek Kaca Transparan) -->
        <div class="flex w-full max-w-6xl bg-[#141519]/90 backdrop-blur-xl rounded-[2rem] overflow-hidden shadow-2xl border border-white/10">
            ...
            <div class="w-full lg:w-[55%] p-8 sm:p-12 lg:p-16 flex flex-col justify-between relative z-10">
                ...
                <!-- Header Text -->
                <div class="mb-10">
                    <p class="text-slate-400 text-[10px] font-bold tracking-widest uppercase mb-4">ACCOUNT SECURITY</p>
                    <h1 class="text-4xl sm:text-5xl font-bold text-white mb-4 tracking-tight">
                        Reset password<span class="text-blue-500">.</span>
                    </h1>
                ...
                <!-- Form Pengisian -->
                <form method="POST" action="{{ route('password.store') }}" class="mt-2 space-y-5">
                    @csrf
                    ...
                    <div class="relative group">
                        ...
                        <!-- Email dimunculkan (pre-filled) agar user tahu akun mana yang di-reset -->
                        <input type="email" name="email" value="{{ old('email', $request->email) }}" class="w-full bg-[#1b1c21]/80 border border-white/5 rounded-xl px-4 py-3.5 text-sm text-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="name@quasar.com" required readonly>
                        <div class="absolute right-4 top-[38px] text-slate-600 transition">
                    ...
                    <!-- Password Baru -->
                    <div class="relative group">
                        <label class="block text-[10px] uppercase tracking-wider text-slate-500 font-bold mb-2 ml-1">New Password</label>
                        <input type="password" name="password" class="w-full bg-[#1b1c21]/80 border border-blue-500/40 rounded-xl px-4 py-3.5 text-sm text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors shadow-[0_0_15px_rgba(59,130,246,0.1)]" placeholder="Minimal 8 karakter" required autofocus autocomplete="new-password">
                    ...
                    <!-- Konfirmasi Password -->
                    <div class="relative group">
                        <label class="block text-[10px] uppercase tracking-wider text-slate-500 font-bold mb-2 ml-1">Confirm New Password</label>
                        <input type="password" name="password_confirmation" class="w-full bg-[#1b1c21]/80 border border-white/5 rounded-xl px-4 py-3.5 text-sm text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="Ulangi password baru" required autocomplete="new-password">
                        @if($errors->has('password_confirmation'))
                            <p class="mt-1 text-xs text-rose-500">{{ $errors->first('password_confirmation') }}</p>
                        @endif
                    </div>
                ...
                <!-- Spacer untuk menyeimbangkan layout agar setara dengan Register/Login -->
                <div class="mt-10"></div>
            </div>
